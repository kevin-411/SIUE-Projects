...
static Cond conds[MAX_CONDS]; //All condition variabls in the system
...
lock_t LockCreate()
{
  ...
  uint32 intrval;
  intrval = DisableIntrs();
  for(lock=0; lock < MAX_LOCKS; lock++){
    if(locks[lock].inuse==0){
      locks[lock].mutex = SemCreate(1);
      locks[lock].inuse=1;
      break;
    }
  }
...
{  
  ...
  if(lock>=0&&lock<MAX_LOCKS){
    intrval = DisableIntrs();
    if(GetCurrentPid()==locks[lock].mypid){
      RestoreIntrs(intrval);
      return 0;
    }
    else if(locks[lock].inuse==1){
      SemHandleWait(locks[lock].mutex);
      locks[lock].mypid=GetCurrentPid();
      RestoreIntrs(intrval);
      return 0;
    }
    RestoreIntrs(intrval);
...
LockHandleRelease(lock_t lock)
{
  if(lock>=0&&lock<MAX_LOCKS){
    if(GetCurrentPid()==locks[lock].mypid){
      locks[lock].mypid=-1;//Not sure if this is needed but it feels right lol
      SemHandleSignal(locks[lock].mutex);
       return 0;
     }
    return 1;
   }else{
...
{  
  if(cond>=0&&cond<MAX_CONDS){
    if(GetCurrentPid()==locks[conds[cond].lock].mypid){
      LockHandleRelease(conds[cond].lock);
      SemHandleWait(conds[cond].sem);
      LockHandleAcquire(conds[cond].lock);
    return 0;
    }
    return 1;
  }else{
...
{
  if(cond>=0&&cond<MAX_CONDS){
    SemHandleSignal(conds[cond].sem); 
    return 0;
  }else{
    return 1;
  }
...
CondHandleBroadcast(cond_t cond)
{
  if(cond>=0&&cond<MAX_CONDS){
  int nodeCount;
    nodeCount=sems[conds[cond].sem].count;
    while(nodeCount!=0){
	  SemHandleSignal(conds[cond].sem);
	  nodeCount++;
          }
    if(sems[conds[cond].sem].count==0){
      return 0;
    }
    return 1;
  }else{
