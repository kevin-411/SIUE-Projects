	     ...
	     uint32 *trapArgs)
{
  ...
  if (cause & TRAP_TRAP_INSTR) {
    ...
    case TRAP_PROCESS_FORK:
      ...
      //ProcessFork(VoidFunc func, uint32 param, filename, int isUser);
      //still need to figure out the rest of the parameters for Processfork
      ProcessSchedule();
      break;
    case TRAP_GETPID:
      dbprintf ('t', "Got a get pid trap!\n");
      GetPid();
      ProcessSchedule();
      break;
