...
//------------------------------------------
  ...
  for (i = 0; i < L1_MAX_ENTRIES; i++) { 
    if(pcb->pagetable[i] & MEMORY_PTE_VALID == 1)
    {
        printf("Before freeing resource,  npages: %d\n",pcb->npages);
        MemoryFreePte (pcb->pagetable[i]);
	pcb->npages --;
	printf("After freeing resource, npages: %d\n",pcb->npages);
	// pcb->pagetable[i] &= MEMORY_PTE_VALID;
    }
  }
...
//------------------------------------------------------------
  ...
  pcb->pagetable[63] = MemorySetupPte(newPage);
 
  //Allocate 1 system stack page
  printf("Allocating sysStack\n");
  newPage = MemoryAllocPage ();
  if (newPage == 0) {
...
//----------------------------------------------
  ...
  stackframe[PROCESS_STACK_PTSIZE] = 64;
...
void PageFaultHandler()
{
  ...
  {
    newPage = MemoryAllocPage();
    currentPCB->npages = (currentPCB->npages) + 1;// move to after allocation!!!!
    if(newPage == 0)
    {
        printf("aFATAL: couldn't allocate memory - no free pages!\n");
        exitsim(); //NEVER RETURNS!
    }
    currentPCB->pagetable[faultaddress / MEMORY_PAGE_SIZE] = MemorySetupPte(newPage);
    printf("Process id: %d\tPage fault address: 0x%x\tPhysical page number: %d\n",pID,faultaddress,newPage); 
  }
  else
  {
