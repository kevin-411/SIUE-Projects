...
//------------------------------------------
  ...
  for(i = 0; i < L1_MAX_ENTRIES; i++) { 
 	for(j = 0; j < L2_MAX_ENTRIES; j++) {
 	  if((pcb->pagetable[i] + (4*j)) & MEMORY_PTE_VALID == 1){
 	       MemoryFreePte (pcb->pagetable[i] + (j));        	
 	       *((uint32*)(pcb->pagetable[i] & MEMORY_PTE_MASK) + j) &=
 	          MEMORY_PTE_VALID; 
 	  }
 	}
  }
  ...
  dbprintf('p',"Leaving ProcessFreeResources\n");
}
...
//------------------------------------------------------------
  ...
  //Sets all entries in the L1 table to 0
  printf("Setting up the L1 Page Table\n");
  for(j = 0; j < L1_MAX_ENTRIES; j++){
    pcb->pagetable[j] = 0;
  }

  //Allocate 3 Text/Data Pages
  printf("Allocating 3 Text/Data Pages\n");
  ...
  tempAddress = pcb->pagetable[0] & MEMORY_PTE_MASK;
  for(i = 0; i < 3; i++){
    newPage = MemoryAllocPage();
    pcb->npages += 1;
    if (newPage == 0) {
     ...
     //Allocate a page to the L2 table
    *((uint32*) tempAddress + (i * 4)) = MemorySetupPte(newPage);
    printf("tempAddress = %i \n",tempAddress);
  }
  
   //Allocate the User Stack Page
  printf("Allocating User Stack\n");
  newPage = MemoryAllocPage ();
  if (newPage == 0) {
    printf ("aFATAL: couldn't allocate memory - no free pages!\n");
    exitsim ();// NEVER RETURNS!
  ...
  tempAddress = pcb->pagetable[L1_MAX_ENTRIES - 1] & MEMORY_PTE_MASK;
  newPage = MemoryAllocPage();
  pcb->npages = 1;
  if (newPage == 0) {
    printf ("aFATAL: couldn't allocate memory - no free pages!\n");
  ...
  //Allocate a page to the L2 table
  *((uint32*) tempAddress + ((L2_MAX_ENTRIES - 1) * 4)) = MemorySetupPte(newPage);  

  //Allocate the System Stack Page 
  newPage = MemoryAllocPage ();
  if (newPage == 0) {
...
{
  // add your code below
  dbprintf ('p',"Entering ProcessKill\n");
  QueueRemove(&pcb->l);
  ProcessFreeResources (pcb);
  ProcessSchedule ();
  dbprintf ('p',"Leaving ProcessKill\n");
}
...
void PageFaultHandler()
{
  ...
  printf("Now entering PageFaultHandler\n");
  
  //Set tempstackframe and faultaddress
  tempstackframe = currentPCB->currentSavedFrame ;
  faultaddress = tempstackframe[PROCESS_STACK_FAULT];
  
  //Set L1 and L2 indices
  L1index = faultaddress >> MEMORY_L1_PAGE_SIZE_BITS;
  L2index = (faultaddress % (1 << MEMORY_L1_PAGE_SIZE_BITS)) >> 
    MEMORY_L2_PAGE_SIZE_BITS;
 
  pID =  GetCurrentPid();

  if(currentPCB->npages < 128){
    tempAddress = currentPCB->pagetable[L1index] & MEMORY_PTE_MASK;
    if(currentPCB->pagetable[L1index] == 0)
	{
      newPage = MemoryAllocPage();
      if (newPage == 0) {
	    printf ("aFATAL: couldn't allocate memory - no free pages!\n");
      ...
      currentPCB->pagetable[L1index] = MemorySetupPte(newPage);
    }
    newPage = MemoryAllocPage();
    if (newPage == 0) {
      printf ("aFATAL: couldn't allocate memory - no free pages!\n");
	...
	//Adding the page to the appropriate entry in the L2 entry.	
    *((uint32*) tempAddress + (L2index)) = MemorySetupPte(newPage);
    currentPCB->npages ++;
          
  }
  else
  {
     printf("Out of memory, killing process with PID: %d \n",pID);
