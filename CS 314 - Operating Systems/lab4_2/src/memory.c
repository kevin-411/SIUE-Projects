...
//-------------------------------------------
    ...
    int offset = addr % (1 << MEMORY_L2_PAGE_SIZE_BITS);
	uint32 L1index = addr >> MEMORY_L1_PAGE_SIZE_BITS;
	uint32 L2index = (addr % (1 << MEMORY_L1_PAGE_SIZE_BITS)) >> MEMORY_L2_PAGE_SIZE_BITS;
	printf("Page:%i\tOffset:%i\tL1index:%i\tL2index:%i\n", page, offset, L1index, L2index);
    if ((L1index > L1_MAX_ENTRIES) || (pcb->pagetable[L1index] & MEMORY_PTE_VALID == 0)||(L2index > L2_MAX_ENTRIES)
      ||(*((uint32*)(pcb->pagetable[L1index] + L2index))  & MEMORY_PTE_VALID ==0)) {
      printf("Memory translate user to system returning zero!\n");
      return (0);
    }
return ((*((uint32*)(pcb->pagetable[L1index] & MEMORY_PTE_MASK) + L2index) & MEMORY_PTE_MASK) + offset);
}

//----------------------------------------------------------------------
//
