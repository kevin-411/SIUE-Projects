...
public class InsertQuery extends AbstractQuery {
    ...
    public int insertTask(Task task) throws SQLException {
    	PreparedStatement insertStmt = getConnection().prepareStatement("INSERT INTO task_table(assignment_id, "
    			+ "question_number, question_text, question_answer, question_type) VALUES(?,?,?,?,?);", Statement.RETURN_GENERATED_KEYS); 
    ...
    public int insertAssignment(Assignment assignment) throws SQLException {
    	PreparedStatement insertStmt = getConnection().prepareStatement("INSERT INTO assignment_table(class_id, "
    			+ "assignment_name) VALUES(?,?);", Statement.RETURN_GENERATED_KEYS); 
    ...
    public int insertOutcomes(Outcome outcome) throws SQLException {
    	PreparedStatement insertStmt = getConnection().prepareStatement("INSERT INTO outcome_table(task_id, "
    			+ "student_id, skill_id, outcome_value) VALUES(?,?,?,?);", Statement.RETURN_GENERATED_KEYS); 
    ...
    public int insertClass(BasicClass clas) throws SQLException {
    	PreparedStatement insertStmt = getConnection().prepareStatement("INSERT INTO class_table(semester,"
    			+ " calander_year, class_name, max_score, user_id, last_used_timestamp) "
        ...
        insertStmt.setInt(5, clas.getUserID());
        int i = insertStmt.executeUpdate();
        if (i > 0)
            return insertStmt.getGeneratedKeys().getInt(1);
		return -1;
    ...
    public int insertStudentClass(Student student, int classID) throws SQLException {
    	PreparedStatement insertStmt = getConnection().prepareStatement("INSERT INTO student_table(first_name, "
    			+ "last_name, middle_name, email_address, student_id_code) VALUES(?,?,?,?,?);", Statement.RETURN_GENERATED_KEYS); 
    ...
    public int insertUser(User user) throws SQLException {
    	int i = 0;
    	PreparedStatement insertStmt = getConnection().prepareStatement("INSERT INTO users_table(user_name, "
    			+ "user_password, question_text, answer_text) VALUES(?,?,?,?);", Statement.RETURN_GENERATED_KEYS); 
    	
    	try {
    		insertStmt.setString(1, user.getUserName());
			insertStmt.setString(2, getSaltedHash(user.getPassword()));
			insertStmt.setString(3, user.getQuestion());
			insertStmt.setString(4, user.getAnswer());
    ...
    public int insertSkill(Skill skill) throws SQLException {
    	PreparedStatement insertStmt = getConnection().prepareStatement("INSERT INTO skill_table(skill_name, "
    			+ "prefilled_outcome_value, class_id, parent_id) VALUES(?,?,?,?);", Statement.RETURN_GENERATED_KEYS); 
