...
public class Login extends AbstractModel {
     ...
     * @throws SQLException
     */
    public LoginStatus verifyUser(TextField username, PasswordField password) throws SQLException
    {
        String usernameString = username.getCharacters().toString();
        String passwordString = password.getCharacters().toString();
        
        if (!verifyText(usernameString, passwordString)) {
            return LoginStatus.INVALID_INPUT;
        }

        if (isLogin) {
            if (facade.getSelect().validatePassword(usernameString, passwordString)) {
                
            	userSettings.setCurrentUserID(facade.getSelect().getUserID(usernameString));
            ...
            if (facade.getSelect().validateUser(usernameString)) {
                User user = new User();
